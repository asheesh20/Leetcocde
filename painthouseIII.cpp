...
public:
    ...
    int solve(int i, int target, int prevColor) {
        ...
        if(target == -1) return 1e8;
        
        if(i != -1 && target != -1 && prevColor != -1 && dp[i][target][prevColor] != -1)  return dp[i][target][prevColor];
        if(houses[i] != -1) {
        ...
        if(i != -1 && target != -1 && prevColor != -1) dp[i][target][prevColor] = ans;
        return  ans;
    }
